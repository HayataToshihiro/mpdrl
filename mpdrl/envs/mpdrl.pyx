...
cdef list make_collision_map(list grid,int radius):
    pbar = tqdm(total=100)
    cdef int n = len(grid)
    cdef list collision = [[False for row in range(n)] for col in range(n)]
    for i in range(n):
        for j in range(n):
            if grid[i][j] == 1:
                collision[i][j] = True
...
class MpdrlEnv(gym.Env):
  ...
  def step(self, action):
    cdef double reward
    #pose update
    self.pose[0] = self.pose[0] + (action[0] * cos(self.pose[2])) * self.DT
    self.pose[1] = self.pose[1] + (action[0] * sin(self.pose[2])) * self.DT
  ...
  def get_reward(self):
    ...
    if cabs(self.pre_dis-self.dis) < 1e-6:
      reward -=0.05
    self.pre_dis = self.dis
    reward -= 1./(200.*math.pi)*cabs(angle_diff(atan2((self.pose[1]-self.target[1]),(self.pose[0]-self.target[0])),self.pose[2]))
    return reward

  def is_movable(self, list pose):
    cdef int i, j
    i = int(pose[0]*self.MAP_RESO)
    j = int(pose[1]*self.MAP_RESO)
  ...
  def raycasting(self, list pose, double angle):
    ...
    x1 = int((pose[0]+self.RANGE_MAX * cos(pose[2]+angle))*self.MAP_RESO)
    y1 = int((pose[1]+self.RANGE_MAX * sin(pose[2]+angle))*self.MAP_RESO)
    steep = False
  ...
  def target_available(self, list pose):
    cdef double min_dis, margin_collision, angle, dis, goal_dis
    cdef int i, NUM_CHECK
    min_dis = self.RANGE_MAX
    margin_collision = self.robot_radius * 0.5
    NUM_CHECK = 36
    for i in range(NUM_CHECK):
      ...
      if min_dis > dis:
        min_dis = dis
    goal_dis = sqrt((pose[0]-self.pose[0])*(pose[0]-self.pose[0])+(pose[1]-self.pose[1])*(pose[1]-self.pose[1]))
    return (min_dis < self.robot_radius + margin_collision) or (goal_dis < self.robot_radius) or (self.is_collision(pose))
